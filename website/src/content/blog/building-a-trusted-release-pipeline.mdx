...
The AppImage previously contained an icon link that pointed back to a temporary GitHub Actions runner path. The application could launch directly while installation through AppManager failed. That bug is now handled in the workflow: the AppImage is repaired, its links are checked and the finished artifact is installed through AppManager before it can reach a release.

This is the kind of problem a successful compilation will not catch. A package needs to be used the way a user will use it.
