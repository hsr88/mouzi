...
2. **Mode:** Silent moves automatically; Manual waits for **Organize now**; Paused does neither.
3. **Grace period:** Silent mode waits five minutes by default. Organize now bypasses this delay.
...
Public version 0.1.6 is unsigned with Authenticode and can trigger SmartScreen. Verify that the file came from [the official download page](/download/windows). Read [Mouzi on Windows](/docs/windows) before choosing **Run anyway**.
