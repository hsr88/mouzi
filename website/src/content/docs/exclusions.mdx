...
Inline comments are supported in Mouzi 0.1.6 and a literal hash can be escaped as `\#`. For example:
...
Choosing **Organize now** or running a scheduled pass bypasses the grace period but still respects lock checking and exclusions.
